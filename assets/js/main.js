...
function scrollActive() {
  ...
  sections.forEach((current) => {
    ...
    // Prefer an explicit `data-nav` attribute for mapping to a nav link; fallback to section id
    const targetNavId =
      current.dataset && current.dataset.nav ? current.dataset.nav : sectionId;

    const targetLink = document.querySelector(
      '.nav__menu a[href*="#' + targetNavId + '"]'
    );
...
// Rely on scroll-only mapping. Ensure correct state on load and resize.
...
// Attach hover handlers to sections that define a data-nav mapping
sections.forEach((section) => {
  const map =
    section.dataset && section.dataset.nav ? section.dataset.nav : null;
  if (!map) return;

  section.addEventListener("mouseenter", () => {
    hoverSuppress = true;
    setNavActiveById(map);
  });

  section.addEventListener("mouseleave", () => {
    // small delay to avoid race with fast scroll
    setTimeout(() => {
      hoverSuppress = false;
      scrollActive();
    }, 150);
...
// Nav-link hover: make hovered nav item show active state temporarily
...
const iconTheme = "uil-sun"; /*from <i class="uil uil-sun"></i>*/
